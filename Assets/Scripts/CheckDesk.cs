...
public class CheckDesk : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (fps != null && bag.Get_Desk() > 0 && i < material.Length)
        {
           ...
           SetMaterialOpaque(material[i]);
            ...
            if(i == 5) // или i == material.Length, если их ровно 5
            {
                Debug.Log(" Все 5 предметов сданы! Переход на следующую сцену...");
    ...
    /// <remarks>Вызывается автоматически через Invoke после сдачи 5-го предмета.</remarks>
    /// </summary>
    private void LoadNextScene()
    {
        if (string.IsNullOrEmpty(nextSceneName))
        {
            Debug.LogError("❌ ОШИБКА: Не указано имя следующей сцены в Inspector!");
            return;
        }
